...
import type { GPTAnalysis, AnalysisHistoryItem, AnalysisData } from '../types';
...
const RelationshipAnalysisContent: React.FC<Props> = ({ analysis, isLoading = false }) => {
  ...
  if (analysis.type === 'individual' && 'insight' in analysis && analysis.insight) {
    return (
      <Box>
  ...
  const analysisData: AnalysisData = 'analysis' in analysis ? analysis.analysis : analysis;
  const {
    overallHealth,
    strengths = [],
    challenges = [],
    ...
    <Box>
      <Box sx={{ 
        ...
        <Box sx={{ position: 'relative', display: 'inline-flex', mb: 2 }}>
          <CircularProgress
            variant="determinate"
            value={typeof overallHealth === 'number' ? overallHealth : 0}
            size={80}
            thickness={4}
            sx={{
              color: (theme) =>
