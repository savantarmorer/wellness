...
const RelationshipAnalysisContent: React.FC<Props> = ({ analysis, isLoading = false }) => {
  ...
  try {
    ...
    const analysisData = isGPTAnalysis(analysis) 
      ? analysis.analysis as GPTAnalysisData
      : analysis as ServiceAnalysis;

    // Extrai os dados com verificações de null/undefined
    const overallHealth = isGPTAnalysis(analysis)
      ? analysisData.overallHealth
      : analysisData.overallHealth.score;
      ...
      <Box>
        ...
        <List sx={{ width: '100%' }}>
          ...
          {Object.entries(categories).map(([category, data]) => {
            const categoryData = data as { score: number; trend: string; insights: string[] };
            return (
                    ...
                    secondary={
                      <Box sx={{ mt: 1 }}>
                        <Box sx={{ mb: 1 }}>
                          ...
                          >
                            Pontuação: {categoryData.score}/10
                          </Typography>
                        </Box>
                        {Array.isArray(categoryData.insights) && categoryData.insights.map((insight: string, index: number) => (
                          <Typography 
                            key={index} 
                            component="div" 
                            sx={{ 
