...
const getProgressValue = (score: number): number => {
  // Temporal analysis scores are on a 0-10 scale
  return (score / 10) * 100;
};
    ...
    <Box sx={{ p: 2 }}>
      ...
      <Paper elevation={3} sx={{ p: 2 }}>
        ...
        <Box sx={{ mb: 3 }}>
          ...
          {analysis.timeframes.daily.discrepancies?.map((discrepancy, index) => (
            <Alert 
              key={`daily-discrepancy-${index}`} 
              severity={discrepancy.significance === 'high' ? 'error' : discrepancy.significance === 'medium' ? 'warning' : 'info'}
              sx={{ mt: 1 }}
            >
              <Typography>
                {discrepancy.category}: Diferença de {discrepancy.difference.toFixed(1)} ({discrepancy.significance})
              </Typography>
            </Alert>
          ))}
          {analysis.timeframes.daily.insights?.map((insight, index) => (
            <Alert key={`daily-insight-${index}`} severity="info" sx={{ mt: 1 }}>
              <Typography>
                {insight.description}
              </Typography>
            </Alert>
        ...
        <Box sx={{ mb: 3 }}>
          ...
          {analysis.timeframes.weekly.discrepancies?.map((discrepancy, index) => (
            <Alert 
              key={`weekly-discrepancy-${index}`} 
              severity={discrepancy.significance === 'high' ? 'error' : discrepancy.significance === 'medium' ? 'warning' : 'info'}
              sx={{ mt: 1 }}
            >
              <Typography>
          ...
          {analysis.timeframes.weekly.insights?.map((insight, index) => (
            <Alert key={`weekly-insight-${index}`} severity="info" sx={{ mt: 1 }}>
              <Typography>
                {insight.description}
              </Typography>
            </Alert>
        ...
        <Box>
          ...
          {analysis.timeframes.monthly.discrepancies?.map((discrepancy, index) => (
            <Alert 
              key={`monthly-discrepancy-${index}`} 
              severity={discrepancy.significance === 'high' ? 'error' : discrepancy.significance === 'medium' ? 'warning' : 'info'}
              sx={{ mt: 1 }}
            >
              <Typography>
          ...
          {analysis.timeframes.monthly.insights?.map((insight, index) => (
            <Alert key={`monthly-insight-${index}`} severity="info" sx={{ mt: 1 }}>
              <Typography>
                {insight.description}
              </Typography>
            </Alert>
