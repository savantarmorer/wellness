...
import { Calendar as BigCalendar, dateFnsLocalizer, Messages } from 'react-big-calendar';
...
const calendarMessages: Messages<DateEvent> = {
  next: 'Próximo',
  previous: 'Anterior',
  today: 'Hoje',
  month: 'Mês',
  ...
  showMore: (total: number) => String(`+${total} mais`),
};
    ...
    <Box>
      ...
      >
        <BigCalendar
          ...
          popup
          components={{
            toolbar: (toolbarProps: ToolbarProps<DateEvent, object>) => (
              <div className="rbc-toolbar">
                ...
                {!isMobile && (
                  <span className="rbc-btn-group">
                    {Object.keys(toolbarProps.views).map(name => (
                      <button
                        key={name}
                        type="button"
                        onClick={() => toolbarProps.onView(name as View)}
                        className={toolbarProps.view === name ? 'rbc-active' : ''}
                      >
                        {String(calendarMessages[name as keyof typeof calendarMessages])}
                      </button>
                    ))}
                  </span>
                )}
