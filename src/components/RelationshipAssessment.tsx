...
interface AssessmentData {
  ...
  context: {
    ...
    support: {
      ...
      supportType: ('emotional' | 'practical' | 'informational')[];
    };
    activities: {
      didActivity: boolean;
      type: string[];
...
const calculateDASTotal = (das: Partial<DyadicAdjustmentScale>): number => {
  ...
  console.log('📊 Debug - DAS Score Calculation:', {
    raw: {
      consenso,
      satisfacao,
      coesao,
    ...
    total: consenso + satisfacao + coesao + expressaoAfetiva,
    ranges: {
      satisfacao: {
...
export const RelationshipAssessment: React.FC<Props> = ({ onSubmit, initialData }) => {
  ...
  const handleSliderChange = (field: keyof AssessmentData) => (_: unknown, value: number | number[]) => {
  ...
  const handleContextChange = (
    section: keyof AssessmentData['context'],
    field: string,
    ...
    <Paper elevation={0} sx={{ p: 3, bgcolor: 'background.default' }}>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <FormGroup>
          <FormControlLabel
            control={
              <Checkbox
                checked={assessmentData.context.communication.hadMeaningfulTalk}
                onChange={(e) => handleContextChange('communication', 'hadMeaningfulTalk', e.target.checked)}
              />
            }
            label="Tivemos uma conversa significativa"
          />
          <FormControlLabel
            control={
              <Checkbox
                checked={assessmentData.context.communication.feltUnderstood}
                onChange={(e) => handleContextChange('communication', 'feltUnderstood', e.target.checked)}
              />
            }
            label="Me senti compreendido(a)"
          />
        </FormGroup>
        <Box sx={{ mt: 2 }}>
          <FormControl fullWidth>
            <FormLabel>Qualidade da Comunicação</FormLabel>
            <Slider
              value={assessmentData.context.communication.quality}
              onChange={(_, value) => handleContextChange('communication', 'quality', value as number)}
              min={0}
              max={10}
              marks
              valueLabelDisplay="auto"
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <FormGroup>
          <FormControlLabel
            control={
              <Checkbox
                checked={assessmentData.context.conflict.hadConflict}
                onChange={(e) => handleContextChange('conflict', 'hadConflict', e.target.checked)}
              />
            }
            label="Tivemos algum conflito"
          />
          <FormControlLabel
            control={
              <Checkbox
                checked={assessmentData.context.conflict.resolvedSameDay}
                onChange={(e) => handleContextChange('conflict', 'resolvedSameDay', e.target.checked)}
              />
            }
            label="Resolvemos no mesmo dia"
          />
        </FormGroup>
        <Box sx={{ mt: 2 }}>
          <FormControl fullWidth>
            <FormLabel>Impacto no Humor</FormLabel>
            <Slider
              value={assessmentData.context.conflict.impactOnMood}
              onChange={(_, value) => handleContextChange('conflict', 'impactOnMood', value as number)}
              min={0}
              max={10}
              marks
              valueLabelDisplay="auto"
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <FormGroup>
          <FormControlLabel
            control={
              <Checkbox
                checked={assessmentData.context.support.neededSupport}
                onChange={(e) => handleContextChange('support', 'neededSupport', e.target.checked)}
              />
            }
            label="Precisei de suporte"
          />
          <FormControlLabel
            control={
              <Checkbox
                checked={assessmentData.context.support.receivedSupport}
                onChange={(e) => handleContextChange('support', 'receivedSupport', e.target.checked)}
              />
            }
            label="Recebi suporte"
          />
        </FormGroup>
        <Box sx={{ mt: 2 }}>
          <FormControl component="fieldset">
            <FormLabel component="legend">Tipo de Suporte</FormLabel>
            <FormGroup row>
              <FormControlLabel
                control={
                  <Checkbox
                    checked={assessmentData.context.support.supportType.includes('emotional')}
                    onChange={(e) => {
                      const currentTypes = assessmentData.context.support.supportType;
                      const newTypes = e.target.checked
                        ? [...currentTypes, 'emotional']
                        : currentTypes.filter(t => t !== 'emotional');
                      handleContextChange('support', 'supportType', newTypes);
                    }}
                  />
                }
                label="Emocional"
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    checked={assessmentData.context.support.supportType.includes('practical')}
                    onChange={(e) => {
                      const currentTypes = assessmentData.context.support.supportType;
                      const newTypes = e.target.checked
                        ? [...currentTypes, 'practical']
                        : currentTypes.filter(t => t !== 'practical');
                      handleContextChange('support', 'supportType', newTypes);
                    }}
                  />
                }
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    checked={assessmentData.context.support.supportType.includes('informational')}
                    onChange={(e) => {
                      const currentTypes = assessmentData.context.support.supportType;
                      const newTypes = e.target.checked
                        ? [...currentTypes, 'informational']
                        : currentTypes.filter(t => t !== 'informational');
                      handleContextChange('support', 'supportType', newTypes);
                    }}
                  />
                }
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <FormGroup>
          <FormControlLabel
            control={
              <Checkbox
                checked={assessmentData.context.activities.didActivity}
                onChange={(e) => handleContextChange('activities', 'didActivity', e.target.checked)}
              />
            }
            label="Fizemos atividades juntos"
          />
        </FormGroup>
        <Box sx={{ mt: 2 }}>
          <FormControl fullWidth>
            ...
            <Slider
              value={assessmentData.context.activities.enjoyment}
              onChange={(_, value) => handleContextChange('activities', 'enjoyment', value as number)}
              min={0}
              max={10}
              marks
              valueLabelDisplay="auto"
      ...
      <Box sx={{ mb: 3 }}>
        <Typography variant="subtitle1" gutterBottom>
          ...
          Crises e Desafios Significativos
        </Typography>
        <FormGroup>
          <FormControlLabel
            control={
              <Checkbox
                checked={assessmentData.context.crisis.hadSignificantCrises}
                onChange={(e) => handleContextChange('crisis', 'hadSignificantCrises', e.target.checked)}
              />
            }
            label="Enfrentamos uma crise significativa"
          />
          
          {assessmentData.context.crisis.hadSignificantCrises && (
            <>
              <FormControl fullWidth margin="normal">
                <FormLabel>Tipo de Crise</FormLabel>
                <RadioGroup
                  value={assessmentData.context.crisis.crisisType}
                  onChange={(e) => handleContextChange('crisis', 'crisisType', e.target.value)}
                >
                  <FormControlLabel value="communication" control={<Radio />} label="Comunicação" />
                  <FormControlLabel value="trust" control={<Radio />} label="Confiança" />
                  <FormControlLabel value="intimacy" control={<Radio />} label="Intimidade" />
                  <FormControlLabel value="external" control={<Radio />} label="Fatores Externos" />
                  <FormControlLabel value="emotional" control={<Radio />} label="Emocional" />
                  <FormControlLabel value="other" control={<Radio />} label="Outro" />
                </RadioGroup>
              </FormControl>
              
              <FormControl component="fieldset" sx={{ mt: 2 }}>
                <FormLabel component="legend">Nível de Impacto</FormLabel>
                <RadioGroup
                  value={assessmentData.context.crisis.impactLevel}
                  onChange={(e) => handleContextChange('crisis', 'impactLevel', e.target.value)}
                >
                  <FormControlLabel value="low" control={<Radio />} label="Baixo" />
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    checked={assessmentData.context.crisis.attemptedSolutions}
                    onChange={(e) => handleContextChange('crisis', 'attemptedSolutions', e.target.checked)}
                  />
                }
                label="Tentamos algumas soluções"
                sx={{ mt: 2 }}
              />
              
              {assessmentData.context.crisis.attemptedSolutions && (
                <FormControl fullWidth margin="normal">
                  <FormLabel>Soluções Tentadas</FormLabel>
                  <FormGroup>
                    {['conversation', 'counseling', 'timeApart', 'compromise', 'other'].map((solution) => (
                      <FormControlLabel
                        key={solution}
                        control={
                          <Checkbox
                            checked={assessmentData.context.crisis.solutionType.includes(solution)}
                            onChange={(e) => {
                              const currentSolutions = assessmentData.context.crisis.solutionType;
                              const newSolutions = e.target.checked
                                ? [...currentSolutions, solution]
                                : currentSolutions.filter(s => s !== solution);
                              handleContextChange('crisis', 'solutionType', newSolutions);
                            }}
                          />
                        }
                        label={getSolutionLabel(solution)}
                      />
                    ))}
                  </FormGroup>
                </FormControl>
              ...
              <FormControl component="fieldset" sx={{ mt: 2 }}>
                <FormLabel component="legend">Status da Resolução</FormLabel>
                <RadioGroup
                  value={assessmentData.context.crisis.resolutionStatus}
                  onChange={(e) => handleContextChange('crisis', 'resolutionStatus', e.target.value)}
                >
                  <FormControlLabel value="unresolved" control={<Radio />} label="Não resolvido" />
                  <FormControlLabel value="partially_resolved" control={<Radio />} label="Parcialmente resolvido" />
                  <FormControlLabel value="resolved" control={<Radio />} label="Resolvido" />
                </RadioGroup>
              </FormControl>
            </>
          )}
...
const getSolutionLabel = (solution: string) => {
  switch (solution) {
    case 'conversation':
      return 'Conversamos sobre o problema';
    case 'counseling':
      return 'Buscamos ajuda profissional';
    case 'timeApart':
      return 'Tiramos um tempo para reflexão';
    case 'compromise':
      return 'Chegamos a um acordo';
    case 'other':
      return 'Outra solução';
    default:
      return solution;
  }
}; 