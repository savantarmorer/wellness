...
export interface DailyAssessment {
  ...
  context: {
    ...
    crisis: {
      ...
      crisisType: 'communication' | 'trust' | 'intimacy' | 'external' | 'emotional' | 'other';
      attemptedSolutions: boolean;
      solutionType: ('conversation' | 'counseling' | 'timeApart' | 'compromise' | 'other')[];
...
export interface AnalysisMoodEntry extends MoodEntry {
  context: {
    activities: string[];
    triggers: string[];
    location: string;
...
export interface TimeframeSummary extends TimeframeAnalysis {
  averageScores: CategoryRatings;
  discrepancies: DiscrepancyAnalysis[];
  insights: Array<{
    category: string;
...
export interface TemporalAnalysis {
  ...
  timeframes: {
    ...
    weekly: TimeframeSummary;
    monthly: TimeframeSummary;
  };
  seasonality: number;
  volatility: number;
  confidence: number;
...
export interface EmotionalDynamics {
  ...
  conflictResolution: {
    style: 'collaborative' | 'competitive' | 'avoidant';
    effectiveness: number;
    patterns: string[];
    confidence: number;
  };
...
export interface ValidationRecommendation {
  type: 'consistency' | 'completeness' | 'reliability' | 'attachment';
  description: string;
  priority: 'high' | 'medium' | 'low';
  toString: () => string;
}
...
export type AssessmentType = 'daily' | 'weekly' | 'monthly' | 'gottman_metrics' | 'attachment' | 'individual' | 'collective'; 
...
export interface RelationshipGoal {
  ...
  title: string;
  description: string;
  category: 'comunicacao' | 'intimidade' | 'conflitos' | 'emocional' | 'atividades';
  target: number;
  progress: number;
  startDate: string;
  dueDate: string;
...
export interface GoalProgress {
  date: string;
  comunicacaoProgress: number;
  intimidadeProgress: number;
  conflitosProgress: number;
  emocionalProgress: number;
  atividadesProgress: number;
} 

export interface EmotionalSyncEntry {
  mood: MoodEntry['mood'];
  timestamp: string;
} 
